...
from .rnn_wrappers import AttentionWrapper, DecoderPrenetWrapper, ConcatOutputAndAttentionWrapper, \
    BahdanauMonotonicAttention_hccho, LocationSensitiveAttention, GmmAttention
...
class Tacotron():
    ...
    def initialize(self, inputs, input_lengths, num_speakers, speaker_id, mel_targets=None, linear_targets=None,
        ...
        with tf.variable_scope('inference') as scope:
            ...
            # Embeddings(256)
            char_embed_table = tf.get_variable('embedding', [len(symbols), hp.embedding_size], dtype=tf.float32,
            ...
            if self.num_speakers > 1:
                ...
                if hp.model_type == 'deepvoice':
                    if hp.speaker_embedding_size == 1:
                        ...
                        decoder_rnn_init_states = [get_embed(speaker_id, self.num_speakers, hp.dec_rnn_size,
                        ...
                        before_highway = deep_dense(speaker_embed, hp.enc_prenet_sizes[-1])  # (?,128)
                        encoder_rnn_init_state = deep_dense(speaker_embed, hp.enc_rnn_size * 2)  # (?,256)
                        attention_rnn_init_state = deep_dense(speaker_embed, hp.attention_state_size)  # (?256)
            ...
            if hp.attention_type == 'bah_mon':
                ...
                attention_mechanism = LocationSensitiveAttention(hp.attention_size, encoder_outputs,
                                                                 memory_sequence_length=input_lengths)
            elif hp.attention_type == 'gmm':  # GMM Attention
                attention_mechanism = GmmAttention(hp.attention_size, memory=encoder_outputs,
                                                   memory_sequence_length=input_lengths)
            elif hp.attention_type == 'bah_mon_norm_hccho':
                attention_mechanism = BahdanauMonotonicAttention_hccho(hp.attention_size, encoder_outputs,
                                                                       normalize=True)
            ...
            attention_cell = AttentionWrapper(GRUCell(hp.attention_state_size), attention_mechanism,
            ...
            dec_prenet_outputs = DecoderPrenetWrapper(attention_cell, speaker_embed, is_training, hp.dec_prenet_sizes,
            ...
            output_cell = OutputProjectionWrapper(decoder_cell,
                                                  hp.num_mels * hp.reduction_factor)
        ...
        with tf.variable_scope('loss') as scope:
            ...
            if hp.prioritize_loss:  # false
                ...
                self.loss = tf.reduce_mean(mel_loss * expanded_loss_coeff) + \
                            0.5 * tf.reduce_mean(l1 * expanded_loss_coeff) + 0.5 * tf.reduce_mean(
                    l1_priority * expanded_loss_coeff)
                self.linear_loss = tf.reduce_mean(0.5 * (tf.reduce_mean(l1) + tf.reduce_mean(l1_priority)))
            else:
                self.loss = tf.reduce_mean(mel_loss * expanded_loss_coeff) + tf.reduce_mean(
                    l1 * expanded_loss_coeff)
    ...
    def add_optimizer(self, global_step):
        ...
        with tf.variable_scope('optimizer') as scope:
            ...
            if hp.decay_learning_rate_mode == 0:
                ...
                self.learning_rate = hp.tacotron_initial_learning_rate * warmup_steps ** 0.5 * tf.minimum(
                    step * warmup_steps ** -1.5, step ** -0.5)
            elif hp.decay_learning_rate_mode == 1:
                self.learning_rate = hp.tacotron_initial_learning_rate * tf.train.exponential_decay(1., step, 3000,
                                                                                                    0.95)
