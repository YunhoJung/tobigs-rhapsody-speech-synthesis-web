    ...
    for data_dir in data_dirs:
        ...
        if not config.skip_path_filter:
            ...
            new_items = [(path, n) for path, n, n_tokens in items if
                         min_n_frame <= n <= max_n_frame and n_tokens >= hparams.min_tokens]

            if any(check in data_dir for check in ["IU", "kss"]):
...
class DataFeederTacotron(threading.Thread):
    ...
    def __init__(self, coordinator, data_dirs, hparams, config, batches_per_group, data_type, batch_size):
        ...
        if self._hp.main_data_greedy_factor > 0 and any(
                main_data in data_dir for data_dir in self.data_dirs for main_data in
                self._hp.main_data):  # 'main_data': ['']
    ...
    def _enqueue_next_group(self):
        ...
        for batch in batches:
            feed_dict = dict(zip(self._placeholders, _prepare_batch(batch, r, self.rng,
                                                                    self.data_type)))
    ...
    def _get_next_example(self, data_dir):
        ...
        while True:
            ...
            if (self.min_n_frame > data["linear"].shape[0] or data["linear"].shape[0] > self.max_n_frame) or len(
...
def _prepare_batch(batch, reduction_factor, rng, data_type=None):
    ...
    input_lengths = np.asarray([len(x[0]) for x in batch],
                               dtype=np.int32)  # batch_size, [37, 37, 32, 32, 38,..., 39, 36, 30]
...
def _pad_target(t, length):
    ...
    return np.pad(t, [(0, length - t.shape[0]), (0, 0)], mode='constant',
                  constant_values=_pad)  # (169, 80) ==> (length, 80)
